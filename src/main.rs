...
use serde_json::*;
use clap::Parser;
use lib::{core::*, transitions::*, models::{geo_pos::*, date_info::*, general::*}, utils::{validators::*, converters::*}, planet_stations::{match_all_planet_stations_range, BodySpeedSet}};
use extensions::swe::{set_sid_mode};
use actix_web::{get, App, HttpServer, Responder, web::{self, Query, Json}};
use std::path::Path;
...
#[get("/p2")]
async fn progress_synastry(params: Query<InputOptions>) -> impl Responder {
  reset_ephemeris_path();
  let micro_interval = time::Duration::from_millis(30);  
  let loc: String = params.loc.clone().unwrap_or("0,0".to_string());
  let geo = if let Some(geo_pos) = loc_string_to_geo(loc.as_str()) { geo_pos } else { GeoPos::zero() };
